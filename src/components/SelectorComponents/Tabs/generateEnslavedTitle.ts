...
export const generateEnslavedTitle = (
  enslavedData: any,
  enslavedId: string,
): string => {
  const name =
    enslavedData?.documented_name || enslavedData?.modern_name || 'Unknown';
...
export const generateEnslavedDescription = (
  enslavedData: any,
  enslavedId: string,
): string => {
  const name =
    enslavedData?.documented_name ||
    enslavedData?.modern_name ||
    'This individual';
  const age = enslavedData?.age;
  ...
  const postDisembarkLocation =
    enslavedData?.post_disembark_location?.name || '';
