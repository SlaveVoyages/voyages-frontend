...
export const generateVoyageTitle = (
  voyageData: any,
  voyageId: string,
): string => {
  const vesselName = voyageData?.voyage_ship?.ship_name || 'unknown vessel';
  const year =
    voyageData?.voyage_dates?.imp_departed_africa_sparsedate?.year ||
    voyageData?.voyage_dates?.voyage_began_sparsedate?.year ||
    voyageData?.voyage_dates?.imp_voyage_began_sparsedate?.year ||
    'unknown year';
  const placePurchased =
    voyageData?.voyage_itinerary?.imp_principal_place_of_slave_purchase?.name ||
    'unknown origin';
  const placeLanded =
    voyageData?.voyage_itinerary?.imp_principal_port_slave_dis?.name ||
...
export const generateVoyageDescription = (
  voyageData: any,
  voyageId: string,
): string => {
  const year =
  ...
  const embarked =
    voyageData?.voyage_slaves_numbers?.imp_total_num_slaves_embarked;
  const disembarked =
    voyageData?.voyage_slaves_numbers?.imp_total_num_slaves_disembarked;
  const placePurchased =
    voyageData?.voyage_itinerary?.imp_principal_place_of_slave_purchase?.name ||
  ...
  if (enslavers && Array.isArray(enslavers) && enslavers.length > 0) {
    const enslaverNames = enslavers.map((enslaver: any) => {
      const name =
        enslaver?.enslaver_alias?.alias || enslaver?.name || 'unknown';
