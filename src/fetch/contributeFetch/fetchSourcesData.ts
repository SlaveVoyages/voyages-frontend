 ...
 * "Source type" dropdown on the Add Source form.
 */
...
/**
 * Create a source via the document app's `CREATE/` endpoint. Mirrors the legacy
 * "Add Source" form: short ref / full ref / source type. The short ref and
 * source type are matched by name server-side (created if new).
 */
...
/**
 * Create a source type via `SourceTypeCreate/`. The new model has only a name.
 */
export const createSourceType = async (name: string): Promise<SourceType> => {
  const response = await axios.post(
    `${BASEURL}/docs/SourceTypeCreate/`,
    { name },
    {
      headers: {
        Authorization: AUTHTOKEN,
